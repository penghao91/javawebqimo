...
<head>
    ...
    <style>
        ...
        .folder-select-card.active {
            ...
            background: linear-gradient(135deg, rgba(78, 115, 223, 0.05), #ffffff);
        }
...
<script>
    ...
    function moveToFolder(questionnaireId) {
        ...
        folderModal.id = 'folderSelectModal';
            ...
            .then(data => {
                ...
                if (folders.length > 0) {
                    folders.forEach(function(folder) {
                        const count = (folder.questionnaireCount != null ? folder.questionnaireCount : 0);
                        const countText = count + ' 份问卷';
                        const desc = (folder.name === '未分类'
                            ? '系统默认文件夹，当前包含 ' + countText
                            : '当前包含 ' + countText);
                        const iconClass = (folder.name === '未分类' ? 'bi-inbox-fill' : 'bi-folder2');

                        folderCardsHtml +=
                            '<div class="folder-select-card" data-id="' + folder.id + '">' +
                            '<div class="folder-select-card-icon">' +
                            '<i class="bi ' + iconClass + '"></i>' +
                            '</div>' +
                            '<div>' +
                            '<div class="folder-select-card-title">' + folder.name + '</div>' +
                ...
                folderModal.innerHTML =
                    ...
                    '<div class="folder-select-grid" id="folderSelectGrid">' +
                    folderCardsHtml +
                    '</div>' +
                    '</div>' +
                    '<hr class="my-4">' +
                    ...
                    '<h6 class="mb-3">' +
                    '<i class="bi bi-plus-circle me-2"></i>创建新文件夹' +
                    '</h6>' +
                    '<div class="input-group">' +
    ...
    function createNewFolder() {
        ...
        fetch('/folder/api/create', {
            ...
            .then(data => {
                if (!data || !data.success) {
                    alertDiv.className = 'alert alert-danger mt-2';
                    alertDiv.innerHTML = '<i class="bi bi-exclamation-circle-fill me-2"></i>' + (data && data.message ? data.message : '创建失败');
                    alertDiv.style.display = 'block';
                    return;
                }
                ...
                if (!newId || !newName) {
                    ...
                    alertDiv.innerHTML = '<i class="bi bi-exclamation-circle-fill me-2"></i>文件夹已创建，但返回数据不完整，请检查后端。';
                    alertDiv.style.display = 'block';
                    return;
                }
    ...
    function confirmMoveToFolder(questionnaireId) {
        const grid = document.getElementById('folderSelectGrid');
        if (!grid) {
            alert('未找到文件夹列表，请关闭后重试');
            return;
        }

        const activeCard = grid.querySelector('.folder-select-card.active');
        if (!activeCard) {
            alert('请选择一个文件夹！');
            return;
        }

        const folderId = parseInt(activeCard.getAttribute('data-id'), 10);
        if (!folderId || isNaN(folderId)) {
            alert('请选择一个有效的文件夹！');
            return;
        }

        const titleEl = activeCard.querySelector('.folder-select-card-title');
        const selectedFolderName = titleEl ? titleEl.textContent.trim() : '选中文件夹';
        ...
        fetch('/questionnaire/api/moveToFolder', {
            ...
            .then(data => {
                if (data && data.success) {
                    ...
                    alert('移动失败：' + (data && data.message ? data.message : '未知错误'));
                }
            })
            .catch(error => {
